    ...
    stages {
      ...
      stage('Deploy App') {
        steps {
          withCredentials([file(credentialsId: 'kubectl-client-cert', variable: 'client_cert'),
             ...
             sh './kubectl --client-certificate=$client_cert --client-key=$client_key --server https://127.0.0.1:43527 --insecure-skip-tls-verify=true apply -f deployment2.yaml '
          }
        }
      }
      stage('Update Service') {
        steps {
          withCredentials([file(credentialsId: 'kubectl-client-cert', variable: 'client_cert'),
                 file(credentialsId: 'kubectl-client-key', variable: 'client_key')])  {
             sh './kubectl --client-certificate=$client_cert --client-key=$client_key --server https://127.0.0.1:43527 --insecure-skip-tls-verify=true apply -f service.yaml '
          }
        }
      }      
    }
